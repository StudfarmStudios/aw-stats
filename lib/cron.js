...
function dailyNewsFeed() {
  ...
  repositories.round.getRoundsByDate(date, function (err, data) {
    ...
    if (bestRatingDelta.delta > 35) {
      message += " " + utils.capitaliseFirstLetter(bestRatingDelta.username) + " improved his rating from "+ Math.floor(bestRatingDelta.oldRating) +" to "+ Math.floor(bestRatingDelta.newRating) +"."
    }
