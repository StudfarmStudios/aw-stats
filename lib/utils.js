...
exports.getServers = function (callback) {
  ...
  client.on('message', function (msg) {
    ...
    lines.forEach(function (line) {
      ...
      server.currentclients = Number(server.currentclients);
      server.maxclients = Number(server.maxclients);

      servers.push(server);
    });
    client.close();

    cache.set('servers', servers, 30);
    callback(null, servers);
  });
};
...
exports.getServerInfo = function (id, callback) {
  var client = dgram.createSocket("udp4");
  var message = new Buffer("operation=joinserver;serverid=" + id);
  client.send(message, 0, message.length, 16727, "assaultwing.com");
  client.on('message', function (msg) {
