...
exports.kill = function (killer, victim, arena, roundId, pos) {
  if (killer) {
    ...
    update['$inc']["score"] = 2;
    if (arena) {
      update['$inc']["kills.arena." + arena.name] = 1;
    }
...
exports.death = function (victim, killer, arena, pos) {
  if (victim) {
    ...
    update['$inc']["score"] = -1;
    if (arena) {
      update['$inc']["deaths.arena." + arena.name] = 1;
    }
...
exports.suicide = function (victim, arena, roundId, pos) {
  if (victim) {
    ...
    update['$inc']["score"] = -1;
    if (arena) {
      update['$inc']["suicides.arena." + arena.name] = 1;
    }
...
exports.bonus = function (pilot, bonus, arena, roundId, pos) {
  if (pilot) {
    ...
    if (arena) {
      update['$inc']["bonus." + bonus + ".arena." + arena.name] = 1;
    }
