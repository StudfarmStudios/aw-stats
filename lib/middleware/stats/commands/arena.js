  ...
  return function (data, client, next) {
    ...
    if (client.pilots) {
      for(var id in client.pilots) {
        ...
        // Has user left
        if (pilot.currentRound && pilot.currentRound.left) {
          delete client.pilots[id];
        }
      }
    } else {
      client.pilots = {};
  ...
  if (client.arena) {
    if (client.pilots) {
      ...
      for (var id in client.pilots) {
        ...
        var result = {_id: pilot._id, score: pilot.currentRound.score, username: pilot.username, oldRating: oldRating, newRating: newRating};
        if (pilot.currentRound.left) {
          result.left = pilot.currentRound.left;
        }
        results.push();
      }
