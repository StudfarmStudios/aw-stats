...
Pilot.prototype.kill = function (victim, pos) {
  ...
  if (victim) {
    ...
    var change = this._getKValue() * (1 - this._expectedResult(1500));
    if (change > 0) {
      var newRating = this.rating + change;
...
Pilot.prototype.death = function (killer, pos) {
  ...
  if (killer) {
    var change = this._getKValue() * (0 - this._expectedResult(killer.rating));
    if (change < 0) {
      var newRating = this.rating + change;
      this.updateRating(newRating);
    }
...
Pilot.prototype.suicide = function (pos) {
  ...
  var change = this._getKValue() * (0 - this._expectedResult(1500));
  if (change < 0) {
    var newRating = this.rating + change;
    this.updateRating(newRating);
  }
