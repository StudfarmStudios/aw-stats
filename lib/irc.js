...
function learn(command) {
  ...
  learnedCommands.forEach(function (command) {
    classifier.addDocument(command.unrecognizedPath, command.path);
  });
...
function processUnrecognized(req, res) {
  ...
  try {
    ...
    lastAnswers[req.from] = {unrecognizedPath: path, path: command};
    res.send("Hmm... Did you mean " + command + "? If it helped remember to say '!aw thanks' else type '!aw help' if not");
...
addCommand('thanks', function (req, res, next) {
  ...
  if (answer) {
    res.send("Your welcome. I'll try to remember it in the future.")
    learn(answer);
    delete lastAnswers[req.from];
  }
});