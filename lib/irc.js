...
function processRecognized (req, res, next) {
  ...
  for (i = unrecognizedCommands.length - 1; i >= 0; i--) {
    command = unrecognizedCommands[i];
    console.log(command);
    if (command.from == from) {
      classifier.addDocument(command.path, path);
      console.log("Training", command.path, " > ", path);
      classifier.train();
      unrecognizedCommands.splice(i,1);
    }
  }
  next();
}
...
function processUnrecognized (req, res) {
  ...
  try {
    ...
    processMessage(from, to, command);
  } catch (e) {
...
function processMessage(from, to, path) {
  var i, captures, keys, data = {}, res = {to: to}, req = {from: from, query: {}, path: path};
...
addCommand('nextgame', function (req, res, next) {
  processRecognized,
